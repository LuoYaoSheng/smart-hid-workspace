...
产出 `smart-hid-firmware.bin`（约 1.09M，随硬件提交小幅漂移，精确字节数以 releases manifest.json 为准；factory 分区 1536K，分区表 3×1536K + NVS 0x8000）+ bootloader.bin + partition-table.bin + ota_data_initial.bin。
